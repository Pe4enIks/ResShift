...
def get_obj_from_str(root, string, reload=False):
    root_module = root.replace("/", ".")
    module, cls = string.rsplit(".", 1)
    if reload:
        module_imp = importlib.import_module(f"{root_module}.{module}", package=None)
        importlib.reload(module_imp)
    return getattr(
        importlib.import_module(f"{root_module}.{module}", package=None), cls
    )


def instantiate_from_config(root, config):
    if not "target" in config:
        raise KeyError("Expected key `target` to instantiate.")
    return get_obj_from_str(root, config["target"])(**config.get("params", dict()))
