...
from .degradation_bsrgan.bsrgan_light import (
        ...
        transform = thv.transforms.Compose(
            [
                ...
                thv.transforms.Normalize(
                    mean=kwargs.get("mean", 0.5), std=kwargs.get("std", 0.5)
                ),
            ]
        )
...
class BaseDataFolder(Dataset):
    def __init__(
        ...
        file_paths_all = util_common.scan_files_from_folder(
            dir_path, im_exts, recursive
        )
        self.file_paths = (
            file_paths_all if length is None else random.sample(file_paths_all, length)
        )
...
class BSRGANLightDegImageNet(Dataset):
    def __init__(
        ...
        if dir_paths is not None:
            file_paths_all.extend(
                util_common.scan_files_from_folder(dir_paths, im_exts, recursive)
            )
        ...
        if degradation == "bsrgan":
            self.degradation_process = partial(
                degradation_bsrgan, sf=sf, use_sharp=use_sharp
            )
        elif degradation == "bsrgan_light":
            self.degradation_process = partial(
                degradation_bsrgan_variant, sf=sf, use_sharp=use_sharp
            )
        else:
            raise ValueError(
                f"Except bsrgan or bsrgan_light for degradation, now is {degradation}"
            )
    ...
    def __getitem__(self, index):
        ...
        if h < self.gt_size or w < self.gt_size:
            ...
            im_hq = cv2.copyMakeBorder(
                im_hq, 0, pad_h, 0, pad_w, cv2.BORDER_REFLECT_101
            )
        ...
        im_hq = (
            torch.from_numpy((im_hq - self.mean) / self.std)
            .type(torch.float32)
            .permute(2, 0, 1)
        )
        im_lq = (
            torch.from_numpy((im_lq - self.mean) / self.std)
            .type(torch.float32)
            .permute(2, 0, 1)
        )
...
class BSRGANLightDeg(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # degradation
        im_lq, im_hq = degradation_bsrgan_variant(
            im_hq, self.sf, use_sharp=self.use_sharp
        )
        if self.resize_back:
            im_lq = cv2.resize(
                im_lq, dsize=(self.gt_size,) * 2, interpolation=cv2.INTER_CUBIC
            )
            im_lq = np.clip(im_lq, 0.0, 1.0)
...
class BicubicData(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if h < self.gt_size or w < self.gt_size:
            pad_h = max(0, self.gt_size - h)
            pad_w = max(0, self.gt_size - w)
            im_gt = cv2.copyMakeBorder(
                im_gt, 0, pad_h, 0, pad_w, cv2.BORDER_REFLECT_101
            )
        ...
        im_gt = augment(
            im_gt, hflip=self.hflip, rotation=self.rotation, return_status=False
        )
